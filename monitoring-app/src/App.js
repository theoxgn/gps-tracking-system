        ...
        <div className="flex-grow overflow-auto bg-white px-4 py-5 custom-scrollbar">
          ...
          <div className="space-y-3">
            {Object.entries(drivers).map(([id, driver]) => (
              <div 
                key={id}
                className={`p-4 rounded-xl cursor-pointer transition-all border ${activeDriver === id ? 'bg-blue-500 text-white border-blue-400 shadow-md' : 'bg-slate-100 hover:bg-blue-50 border-transparent'} group`}
                onClick={() => handleDriverSelect(id)}
              >
                <div className="flex justify-between items-center">
                  <div className="font-bold text-base truncate">{id}</div>
                  <div className={`text-xs ${activeDriver === id ? 'text-blue-100' : 'text-slate-500'} flex items-center gap-1`}>
                    <Clock size={12} />
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                  <div className="flex items-center gap-1">
                    <MapPin size={13} className={activeDriver === id ? 'text-blue-100' : 'text-blue-500'} />
                    {driver.position[0].toFixed(4)}, {driver.position[1].toFixed(4)}
                  </div>
                  <div className="flex items-center gap-1">
                    <Navigation size={13} className={activeDriver === id ? 'text-blue-100' : 'text-blue-500'} />
        ...
        >
          ...
          {Object.entries(drivers).map(([id, driver]) => (
            <Marker 
              ...
              icon={createCustomIcon(activeDriver === id ? '#3b82f6' : '#64748b', driver.heading)}
            >
              <Popup>
